...
const char DEFAULT_WALLET_FILE_NAME[] = "zeno.wallet";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("zenowallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("zenowallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("zenowallet.desktop");
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.zeno.zenowallet");
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\zeno", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:zeno");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\zeno\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\zeno\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("zenowallet.cfg"));
