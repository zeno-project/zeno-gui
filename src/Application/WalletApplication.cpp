// Copyright (c) 2017-2018, zeno developers
...
const char BYTECOIN_URI_SCHEME_NAME[] = "zeno";
...
bool WalletApplication::init() {
  ...
  m_lockFile = new QLockFile(Settings::instance().getDataDir().absoluteFilePath("zenowallet.lock"));
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      "If you have zenod with non-default RPC port, you should terminate it and relaunch zenoWallet\n"
      "or\n"
      "Set the Local deamon required port in zenoWallet Menu/Preferences/Connection settings."));
	  if (m_splash != nullptr)
		  m_splash->hide();
      dlg.exec();
      return false;
